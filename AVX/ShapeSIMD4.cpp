...
public:
	...
	Shape(vec3* positions, int N)
	{
		...
		int mTotal = (count * 3) / 8 + 3;
		//if (mverts != nullptr)delete[] mverts;
		//mverts = new __m256[mTotal];
		mCount = mTotal;
		index = 0;
		for (unsigned int i = 0; i < count*3; i += 8)
		{
	...
	uint32_t supportPoint(__m256& B, __m256& C, __m256& D) const
	{
		float results[8];
		float magnitude = -999999;
		uint32_t tempID = 0;
		uint32_t indFirst = 0;
		for (int i = 0; i < mCount; i += 3)
		{

			__m256 mults = _mm256_mul_ps(mverts[i], B);
			_mm256_storeu_ps(&results[0], mults);

			float dR = results[0] + results[1] + results[2];
			float dRR = results[3] + results[4] + results[5];
			float temp = results[6] + results[7];
			if (dR > magnitude)
				{
					magnitude = dR;
					tempID = indFirst;
			...
			mults = _mm256_mul_ps(mverts[i + 1], C);
			_mm256_storeu_ps(&results[0], mults);
			...
			indFirst++;
			dR = results[1] + results[2] + results[3];
			dRR = results[4] + results[5] + results[6];
			temp = results[7];
			if (dR > magnitude)
				{
					magnitude = dR;
			...
			mults = _mm256_mul_ps(mverts[i + 2], D);
			_mm256_storeu_ps(&results[0], mults);
			...
			dR = results[2] + results[3] + results[4];
			dRR = results[5] + results[6] + results[7];
