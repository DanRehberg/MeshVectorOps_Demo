...
public:
	...
	uint32_t supportPoint(const vec3& direction) const
	{
		...
		//uint32_t tempID = 0;
		#pragma omp declare reduction(maxSupport : struct IndexDot : omp_out = omp_in.mag > omp_out.mag ? omp_in : omp_out)
		struct IndexDot supportID;
		supportID.mag = 0.0f;
		supportID.index = -1;
		#pragma omp parallel for reduction(maxSupport:supportID)
